...
contract Ballot {
    ...
    function giveRightToVote(address[] memory votees) external {
        require( msg.sender == chairperson, "Only chairperson can give rights.");
        for (uint p = 0; p < votees.length; p++) {
            address voter = votees[p];
    ...
    function winningProposals() public view returns (uint[] memory) {
        ...
        for (uint i = 0; i < proposals.length; i++) {
            if (proposals[i].voteCount > highestVoteCount) {
                ...
                winnerCount = 1; // Reset count for new highest
        ...
        // Allocate exact size
        uint[] memory winners_ = new uint[](winnerCount);
        uint index = 0;
    ...
    function winningNames() external view
