...
use crate::{  handlers::cmds::Command, tests::{self, binary::basic} };

const NAME_TRX: &'static str = "nametrx_bin";

#[tokio::test]
async fn trx() {
    let t = tests::start_test("trx_bin").await;
    ...
    basic::test_recovery(&mut stream, 2).await;

    let new_addr = basic::test_eth_key(&mut stream).await;
    ...
    stream.set_opcode(Command::SendTrx);
    stream.write_address(&a1);
    stream.write_address(&new_addr);
    let amount = U256::from(2) * WEI_IN_ETHER;
    stream.write_buff(amount.as_le_slice());
    stream.flush_buffer(false).await.unwrap();

    if !stream.was_success().await.unwrap() {
        panic!("SEND TRX FAILED: {}", stream.read_str().unwrap());
    }
    let _rec = stream.read_receipt().unwrap();
...
fn format_ether_2dp(wei: U256) -> String {
    // Get whole ETH part
