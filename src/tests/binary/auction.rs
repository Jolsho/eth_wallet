...
use crate::{handlers::{ cmds::Command, json::auction::{DeployContractResponse, TrxReceipt}, stream::FramedStream}, tests::{self, binary::basic}};

const NAME_TRX: &str = "auction_bin";

#[tokio::test]
pub async fn auction() {
    let t = tests::start_test("multis_bin").await;
    ...
    pp.anvil_mine(Some(1),None).await.unwrap();
...
async fn test_bid(
    ...
    s.write_address(&bidder);
    s.write_address(&contract_addr);
    let bid_amount: [u8;32] = U256::from(amount).to_le_bytes();
    s.write_buff(&bid_amount);
    s.flush_buffer(false).await.unwrap();
